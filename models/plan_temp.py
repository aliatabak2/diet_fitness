...
import random  # <-- Random yerine bunu kullanıyoruz
...
class DietDailyPlan(models.Model):
    ...
    def generate_random_meals(self, max_total_kcal=2200, extra_max_kcal=1000):
        ...
        for plan in self:
            ...
            # 1) Zorunlu akşam yemeği (öncelik course='meal')
            dinner = self._random_recipe([("course", "=", "meal")], kcal_cap=max_total_kcal)
            if not dinner:
                dinner = self._random_recipe([], kcal_cap=max_total_kcal)

            if not dinner:
                # En düşük kaloriliyi dene (uyan yoksa gün boş kalmasın)
                dinner = Recipe.search([], order="kcal asc", limit=1)
            ...
            # 2) Kalan kalori için kahvaltı/snack (her biri ≤ extra_max_kcal)
            ...
            while total < max_total_kcal - 1:
                ...
                rec = self._random_recipe([], kcal_cap=single_cap, exclude_ids=used_ids)
                if not rec:
                    break  # uygun tarif kalmadı

                self.env["diet.daily.meal.line"].sudo().create({
                    "plan_id": plan.id,
                    "meal_type": extra_types[idx % len(extra_types)],
                    "recipe_id": rec.id,
                })
        ...
        for rec in self:
            ...
            for where, kcal_target in targets.items():
                ...
                self.env["diet.daily.meal.line"].sudo().create({
                    ...
                    "note": msg,
                    # kcal related & readonly, set etmiyoruz
                })
            rec.state = "ready"
